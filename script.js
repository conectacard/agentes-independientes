...
const CONFIG = {
    ...
    facebook: "https://www.facebook.com/pepe.moreno.9235?locale=es_LA",
    instagram: "https://www.instagram.com/my_sing_studio/",
    maps: "https://maps.app.goo.gl/557uW3JY2ThC4mHm9", 
    youtubeUrl: "https://www.youtube.com/watch?v=VVktqcsyhos",
    textos: {
        cat1: { t: "RESPALDO Y ASESORÍA", c: "Como tu Corredor de Seguros Independiente, mi prioridad no es venderte una marca, sino encontrar la protección perfecta para ti, tu familia o tu empresa. Analizo el mercado por ti para ofrecerte las coberturas más sólidas con las mejores condiciones." },
        cat2: { t: "SERVICIOS", c: "Protegemos lo que más amas a través de soluciones a tu medida: Seguros de Auto con cobertura total, Gastos Médicos Mayores para blindar tu salud, Planes de Vida/Ahorro para tu retiro y protección Patrimonial para empresas y hogares." },
        cat3: { t: "CLIENTES SATISFECHOS", c: "La verdadera valía de un seguro se demuestra en los momentos difíciles. Mi compromiso no termina al entregarte tu póliza; te acompaño personalmente en cada trámite, siniestro o renovación, garantizando tu tranquilidad 24/7." }
    },
    sucursales: {
        suc1: { nombre: "Iniciar Cotización por WhatsApp", wa: "5214491472336", maps: "mailto:altpromex@gmail.com" }
    }
};
...
function renderGallery(cat) {
    ...
    if (cat === 'cat3') {
        ...
        videoContainer.innerHTML = `
            <a href="https://www.youtube.com/shorts/RJZcfcVOVg4" target="_blank" style="background: #000; color: #fff; padding: 10px 20px; border-radius: 8px; text-decoration: none; font-weight: 800; font-size: 0.7rem; border: 1px solid var(--brand-accent);">Opinión de nuestros clientes</a>
            <a href="https://www.youtube.com/shorts/zGmkRorwhic" target="_blank" style="background: #000; color: #fff; padding: 10px 20px; border-radius: 8px; text-decoration: none; font-weight: 800; font-size: 0.7rem; border: 1px solid var(--brand-accent);">El respaldo de tu Asesor</a>
        `;
...
// INYECCIÓN DINÁMICA DEL ACORDEÓN DE CONTACTO OPTIMIZADO PARA ASESORÍA
function inicializarAcordeon() {
    ...
    Object.keys(CONFIG.sucursales).forEach((key, index) => {
        ...
        // Crear Panel Oculto con opciones híbridas WhatsApp / Correo
        const panel = document.createElement('div');
        panel.id = `${key}-panel`;
        panel.className = 'sucursal-panel-content';
        panel.innerHTML = `
            <div class="sucursal-info-block">
                <p class="suc-domicilio"><i class="fas fa-user-tie"></i> Alan Ochoa Durán - Corredor de Seguros</p>
                <p class="suc-horario"><i class="far fa-clock"></i> Atención Inmediata 24/7 en Siniestros</p>
            </div>
            <a href="https://wa.me/${suc.wa}?text=Hola%20Alan!%20Me%20interesa%20solicitar%20una%20cotizaci%C3%B3n%20para%20un%20seguro." target="_blank" class="btn-menu whatsapp"><i class="fab fa-whatsapp"></i> Iniciar por WhatsApp</a>
            <a href="${suc.maps}" class="btn-menu maps-btn" style="background-color: #0077ff;"><i class="fas fa-envelope"></i> Enviar Documentos por Correo</a>
        `;
...
async function shareExperienceRobust() {
    try { await navigator.share({ title: 'Alan Ochoa Durán | Corredor de Seguros', url: window.location.href }); }
